...
const DeliverdOrders = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            try {
                const response = await fetch(`https://api.vitaparapharma.com/api/v1/delivery/out`, {
                    headers: {
    ...
    const handleSubmit = async () => {  //2update new
        ...
        try {
            const response = await fetch(`https://api.vitaparapharma.com/api/v1/delivery/delivered/${customerId}`, {
                method: 'PUT',
                headers: {
                    Authorization: `Bearer ${Authurization}`,
